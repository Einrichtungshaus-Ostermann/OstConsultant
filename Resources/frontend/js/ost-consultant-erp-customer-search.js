...
;(function ($) {
    ...
    $.plugin("ostConsultantErpCustomerSearch", {
        ...
        init: function () {
            ...
            try {
                fully.bind('onQrScanSuccess','fillSearch(\'$code\');');
                me._on(me.$el.find("button.camera--button"), 'click', $.proxy(me.onScanClick, me));
            } catch (e) {}
        ...
        onScanClick: function (event) {
            fully.scanQrCode("Bitte Kundennummer einscannen", "", -1, -1, true, true)
        },
...
function fillSearch(code) {
    $('.panel--body > input[type="text"]').val(code);
}
